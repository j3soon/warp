...
def test_atomic_bitwise_tile(test, device):
    ...
    def test_atomic_bitwise_tile_kernel(a: wp.array(dtype=wp.uint32), b: wp.array(dtype=wp.uint32), op_type: int):
        ...
        if op_type == 0:
            ...
            wp.atomic_and(s, 0, (b[word_idx] & bit_mask) | ~bit_mask)
        # elif op_type == 4:
        #     wp.atomic_or(s, 0, b[word_idx] & bit_mask)
        # elif op_type == 5:
        #     wp.atomic_xor(s, 0, b[word_idx] & bit_mask)
        a[word_idx] = s[0]
    ...
    with wp.ScopedDevice(device):
        ...
        # wp.launch_tiled(test_atomic_bitwise_tile_kernel, dim=n, inputs=[atomic_and_array, target_array, 3], block_dim=32)
        # wp.launch_tiled(test_atomic_bitwise_tile_kernel, dim=n, inputs=[atomic_or_array, target_array, 4], block_dim=32)
        # wp.launch_tiled(test_atomic_bitwise_tile_kernel, dim=n, inputs=[atomic_xor_array, target_array, 5], block_dim=32)
...
add_function_test(TestAtomicBitwiseTile, "test_atomic_bitwise_tile", test_atomic_bitwise_tile, devices=devices)
