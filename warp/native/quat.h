...
template<typename Type>
inline CUDA_CALLABLE void and_inplace(quat_t<Type>& q, int idx, Type value)
{
#ifndef NDEBUG
    if (idx < -4 || idx >= 4)
    {
...
template<typename Type>
inline CUDA_CALLABLE void and_inplace(quat_t<Type>& q, slice_t slice, Type value)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
...
template<unsigned SliceLength, typename Type>
inline CUDA_CALLABLE void and_inplace(quat_t<Type>& q, slice_t slice, const vec_t<SliceLength, Type> &a)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
...
inline CUDA_CALLABLE void adj_and_inplace(
    quat_t<Type>& q, int idx, Type value,
    quat_t<Type>& adj_q, int adj_idx, Type& adj_value
) {}
...
template<typename Type>
inline CUDA_CALLABLE void or_inplace(quat_t<Type>& q, int idx, Type value)
{
#ifndef NDEBUG
    if (idx < -4 || idx >= 4)
    {
...
template<typename Type>
inline CUDA_CALLABLE void or_inplace(quat_t<Type>& q, slice_t slice, Type value)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
...
template<unsigned SliceLength, typename Type>
inline CUDA_CALLABLE void or_inplace(quat_t<Type>& q, slice_t slice, const vec_t<SliceLength, Type> &a)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
...
template<typename Type>
inline CUDA_CALLABLE void xor_inplace(quat_t<Type>& q, int idx, Type value)
{
#ifndef NDEBUG
    if (idx < -4 || idx >= 4)
    {
...
template<typename Type>
inline CUDA_CALLABLE void xor_inplace(quat_t<Type>& q, slice_t slice, Type value)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
...
template<unsigned SliceLength, typename Type>
inline CUDA_CALLABLE void xor_inplace(quat_t<Type>& q, slice_t slice, const vec_t<SliceLength, Type> &a)
{
    assert(slice.start >= 0 && slice.start <= 4);
    assert(slice.stop >= -1 && slice.stop <= 4);
    assert(slice.step != 0 && slice.step < 0 ? slice.start >= slice.stop : slice.start <= slice.stop);
